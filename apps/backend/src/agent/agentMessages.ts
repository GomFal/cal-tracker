...
- If a prior tool result says a nutrition search is awaiting user selection, the app may show candidate rows that are not in your context. When the user refers to one by number or candidate reference, call resolve_candidate_reference and use only the selected item it returns. Do not infer or reconstruct unseen nutrition candidates.
