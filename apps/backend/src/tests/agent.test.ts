...
describe("AgentService", () => {
  ...
  it("falls back to deterministic meal logging when the model returns no tool call for a clear meal request", async () => {
    const { request } = buildTestApp({
      agentProvider: new FakeChatAgentProvider({
        toolCalls: [],
        rawResponse: {},
    ...
    const res = await request("http://localhost/v1/agent/runs", {
      ...
      headers: authHeader,
      body: JSON.stringify({
        text: "Put me 100 grams of chicken breast and 100 grams of bread",
        source: "flutter",
      }),
    });
