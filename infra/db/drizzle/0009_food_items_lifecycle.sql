...
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS food_items_user_custom_active_idx
  ON food_items (user_id, source, updated_at DESC)
  WHERE deleted_at IS NULL AND user_id IS NOT NULL AND source = 'user_custom';
--> statement-breakpoint
...
BEGIN
  ...
  next_search_text := lower(concat_ws(
    ...
    (
      ...
      FROM jsonb_array_elements_text(
        CASE
          WHEN jsonb_typeof(NEW.nutrients_json->'usualFoodAliases') = 'array'
          THEN NEW.nutrients_json->'usualFoodAliases'
          ELSE '[]'::jsonb
        END
      ) AS alias(value)
    )
  ...
  VALUES (
    ...
    next_scope,
    next_search_text,
    to_tsvector('simple', next_search_text),
    next_rank_bucket,
    NEW.source,
    NEW.external_source,
    NEW.data_type,
