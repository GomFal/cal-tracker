...
grep -Fq 'scripts/mobile/verify-apk-signing.sh' "$WORKFLOW" \
  || fail "the mobile workflow does not verify the production APK certificate"
...
grep -Fq 'verify-apk-signing.sh' "$BUILD_SCRIPT" \
  || fail "the Android build does not verify the prod APK certificate"
grep -Fq 'verify-apk-signing.sh' "$DEPLOY_SCRIPT" \
  || fail "prebuilt prod APKs are not verified before publication"
