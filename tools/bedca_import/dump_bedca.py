...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create, validate, and locally load a BEDCA inspection corpus.")
    parser.add_argument(
        "command",
        choices=[
            "dump",
    ...
    ctx = Context(Path(args.data_dir), args.delay, args.target, args.confirm_production)
    ctx.ensure_dirs()
    if args.command == "dump":
        ...
        if ctx.target != "local":
            raise SystemExit("run-local only supports --target local")
        dump(ctx, args.limit)
        validate(ctx)
        build_load(ctx)
        load(ctx)
