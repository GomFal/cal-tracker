    ...
    with gzip.open(ctx.raw_path, "rt", encoding="utf-8", errors="replace") as fh:
        for line in fh:
            ...
            if not rows:
                ...
                continue
            for row in rows:
                food_writer.writerow({column: row.get(column, "") for column in FOOD_LOAD_COLUMNS})
                food_rows += 1
...
def food_rows_for_product(
    ...
    for language in languages:
        ...
        product_serving_grams = serving_grams(product)
        rows.append(
            {
                ...
                "_off_serving_grams": product_serving_grams,
            }
        )
    return rows
...
def portion_row_for_food(food: dict[str, object]) -> dict[str, object] | None:
    grams = number_value(food.get("_off_serving_grams"))
